...
import re
from ..services.lecture_service import get_apply_list, get_enrolment_code, get_enrolments, lecture_listing, make_lecture, modify_enrolment,modify_lecture,delete_lecture, request_enrolment, search_student,major_listing,attendee_add,attendee_listing,lecture_access_check,mod_lecutre_listing
...
class React_Lecture_apply_list(Resource):
    #강의에 신청한 학생 리스트
    @jwt_required()
    def get(self):
        parser = reqparse.RequestParser()
        ...
        lecture_no = args['lecture_no']
        user_info=get_jwt_identity()
        res = get_apply_list(user_info["user_no"],lecture_no)
        return jsonify(res)
